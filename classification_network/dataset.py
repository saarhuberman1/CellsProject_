...
class CellDataset(Dataset):
    ...
    def __init__(self,
        ...
        if self.is_train and 'HE' in train_list_path and not full_im:
            # if train with patches and not full images, dulicate the train list to get more random patches at each epoch.
            self.images += 3 * self.images
            self.labels += 3 * self.labels
            self.ids += 3 * self.ids
        if self.is_train:
    ...
    def __getitem__(self, index: int):
        ...
        if not self.is_bm:
            if self.is_train:
                ...
                return im.float(), label, sid
        else:
            return im, label, sid, im_path
    ...
    def augment(self, im):
        ...
        if random.uniform(0, 1) < self.args.temp_aug_prob:
            ...
            aug = iaa.ChangeColorTemperature((5700, 8000), from_colorspace=iaa.CSPACE_BGR)
        ...
        # crop and resize
        if np.random.uniform(0, 1) < self.args.resize_prob:
            offset_x = np.random.randint(0, int(self.target_size * 0.15))
            offset_y = np.random.randint(0, int(self.target_size * 0.15))
            offset_x_end = np.random.randint(1, int(self.target_size * 0.15))
            offset_y_end = np.random.randint(1, int(self.target_size * 0.15))
            crop_im = im[offset_x:-offset_x_end, offset_y:-offset_y_end]
            im = cv2.resize(crop_im, dsize=(self.target_size, self.target_size), interpolation=cv2.INTER_LINEAR)
...
if __name__ == '__main__':
    root_path = r'C:\Users\amirlivn\OneDrive - Intel Corporation\ER\annotated3\train'
    dataset = CellDataset(root_path, is_train=True, debug=True)
